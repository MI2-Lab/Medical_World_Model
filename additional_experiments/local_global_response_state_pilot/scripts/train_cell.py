#!/usr/bin/env python3
"""Train one gated C1B Local--Global pilot arm/seed/fold."""
...
FORMAL_ARMS = ("GAP0", "GAP3", "LOCAL0", "LOCAL3", "LG0", "LG3")
BASELINE_BY_GROUNDED = {"GAP3": "GAP0", "LOCAL3": "LOCAL0", "LG3": "LG0"}
...
def _validate_initialization_report(
    ...
    *,
    ...
    if not isinstance(per_arm, dict) or not isinstance(per_arm.get(arm), dict):
        raise ValueError("paired initialization report lacks the requested arm")
    ...
    if not shared or shared != arm_shared or shared != observed_shared:
    ...
    if not transition or transition != arm_transition or transition != observed_transition:
...
def main() -> None:
    ...
    pilot_security.require_module_within(
        pilot_model_module, SRC_ROOT / "lg_response_pilot", label="pilot model"
    )
    pilot_security.require_module_within(
        pilot_training_module, SRC_ROOT / "lg_response_pilot", label="pilot training"
    )
    ...
    if tuple(ARMS) != FORMAL_ARMS:
        raise RuntimeError("formal pilot arm inventory drifted")
    ...
    output_dir = resolve_contained_path(
        args.output_dir,
        CHECKPOINT_ROOT,
        label="pilot cell output directory",
    )
