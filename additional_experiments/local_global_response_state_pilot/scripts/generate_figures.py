#!/usr/bin/env python3
"""Render the ten preregistered figures from completed deidentified tables."""
...
def main() -> None:
    ...
    if Path(str(getattr(pilot_security, "__file__", ""))).resolve() != (
        ...
        raise ImportError("pilot security module was shadowed before figure rendering")
    ...
    import lg_response_pilot.figures as pilot_figures_module

    for module, label in (
        (pilot_analysis_module, "pilot analysis"),
        (pilot_figures_module, "pilot figures"),
    ):
    ...
    if not summary_path.is_file():
        raise FileNotFoundError("aggregation_summary.json is required for figure provenance")
    try:
