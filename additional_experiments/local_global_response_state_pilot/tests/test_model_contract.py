...
class ArmAndUpstreamContractTest(unittest.TestCase):
    def test_arm_inventory_and_public_spec_fields(self) -> None:
        ...
        for arm, spec in ARM_SPECS.items():
            ...
            self.assertIn(spec.architecture, {"GAP", "LOCAL", "LOCAL_GLOBAL"})
...
class ProjectionAndTargetContractTest(unittest.TestCase):
    def test_local_global_half_initialization_has_identity_condition(self) -> None:
        gap = build_model("GAP0", 3026).eval()
        local = build_model("LOCAL0", 3026).eval()
        lg = build_model("LG0", 3026).eval()
        gap_linear, gap_norm = gap.response_projection
        local_linear, local_norm = local.response_projection
        lg_linear, lg_norm = lg.response_projection
        self.assertTrue(torch.equal(gap_linear.weight, local_linear.weight))
        self.assertTrue(torch.equal(gap_linear.bias, local_linear.bias))
        self.assertTrue(torch.equal(gap_norm.weight, local_norm.weight))
        self.assertTrue(
            torch.equal(
                2.0 * lg_linear.weight[:, :FINAL_FEATURE_CHANNELS],
                gap_linear.weight,
            )
        )
        self.assertTrue(
        ...
        self.assertTrue(torch.equal(lg_linear.bias, gap_linear.bias))
        self.assertTrue(torch.equal(lg_norm.weight, gap_norm.weight))
        self.assertTrue(torch.equal(lg_norm.bias, gap_norm.bias))

        state = torch.randn(
            (17, FINAL_FEATURE_CHANNELS),
            generator=torch.Generator().manual_seed(3035),
        )
        torch.testing.assert_close(
            lg.response_projection(torch.cat((state, state), dim=-1)),
            gap.response_projection(state),
            rtol=1e-5,
            atol=1e-6,
        )
...
class InitializationAndCheckpointTest(unittest.TestCase):
    ...
    def test_paired_initialization_report_and_parameter_counts(self) -> None:
        ...
        for arm in ARMS:
            ...
            expected_projection = 49_728 if arm.startswith("LG") else 25_152
            self.assertEqual(
    ...
    def test_objective_grounding_and_selected_checkpoint_roundtrip(self) -> None:
        ...
        model = build_model("LG3", 2027)
        payload = {
            "schema_version": 1,
            "stage": "local_global_response_state_pilot",
            "arm": model.arm,
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(loaded.arm, "LG3")
