"""Formal Local--Global pilot training on sealed Stage-B B32 primitives.

Only the arm names and their paired no-grounding baselines differ from the
completed C1B-H Stage-B experiment.  Logical batching, SIGReg, validation,
optimizer cadence, and EMA cadence are imported from that experiment after an
...
def validate_formal_hyperparameters(hyperparameters: TrainHyperparameters) -> None:
    ...
    if observed != dict(FORMAL_HYPERPARAMETERS):
        raise ValueError(
            "formal Local--Global pilot hyperparameters are frozen to physical=4, "
...
def train_epochs(
    *,
    ...
    """Train one of the six C1B arms under the frozen Stage-B selection rule."""
    ...
    if output.exists() and any(output.iterdir()):
        raise FileExistsError(f"refusing to mix or overwrite an existing pilot run: {output}")
    ...
    if tuple(
        ...
        raise ValueError("formal pilot training requires physical=4 and accumulation=8")
