"""Render the ten preregistered public pilot figures from deidentified tables."""
...
FIGURE_FILENAMES = (
    "01_local_global_architecture_schematic.png",
...
ARM_ORDER = ("GAP0", "GAP3", "LOCAL0", "LOCAL3", "LG0", "LG3")
ARM_COLORS = {
    "GAP0": "#6b7280",
    "GAP3": "#374151",
    "LOCAL0": "#2a9d8f",
...
def _arrow(axis: plt.Axes, start: tuple[float, float], end: tuple[float, float]) -> None:
    axis.add_patch(
        FancyArrowPatch(start, end, arrowstyle="-|>", mutation_scale=12, color="#334155")
    )


def architecture_schematic(table1: pd.DataFrame) -> plt.Figure:
    if set(table1["arm"]) != set(ARM_ORDER):
        raise ValueError("architecture schematic requires all six arms")
    figure, axis = plt.subplots(figsize=(11.0, 4.8))
    axis.set_xlim(0, 1)
    axis.set_ylim(0, 1)
    axis.axis("off")
    _box(axis, (0.03, 0.40), 0.17, 0.20, "Final encoder map\n128 × D × H × W", "#4361ee")
    _box(axis, (0.28, 0.68), 0.18, 0.16, "Global GAP\n128-D", "#6b7280")
    _box(axis, (0.28, 0.16), 0.18, 0.16, "Fixed 64-mm local\nfractional cell overlap\n128-D", "#2a9d8f")
    _arrow(axis, (0.20, 0.52), (0.28, 0.76))
    _arrow(axis, (0.20, 0.48), (0.28, 0.24))
    _box(axis, (0.55, 0.68), 0.18, 0.16, "GAP\nLinear 128→192\n+ LayerNorm", "#6b7280")
    _box(axis, (0.55, 0.16), 0.18, 0.16, "LOCAL\nLinear 128→192\n+ LayerNorm", "#2a9d8f")
    _box(axis, (0.53, 0.40), 0.22, 0.16, "LOCAL_GLOBAL\nconcat [local; global]\nLinear 256→192 + LN", "#e9a03b")
    _arrow(axis, (0.46, 0.76), (0.55, 0.76))
    _arrow(axis, (0.46, 0.24), (0.55, 0.24))
    _arrow(axis, (0.46, 0.72), (0.53, 0.52))
    _arrow(axis, (0.46, 0.28), (0.53, 0.44))
    _box(axis, (0.83, 0.40), 0.14, 0.20, "Response state\nrₜ ∈ ℝ¹⁹²\nJEPA / FTV", "#7b2cbf")
    _arrow(axis, (0.73, 0.76), (0.86, 0.60))
    _arrow(axis, (0.73, 0.24), (0.86, 0.40))
    _arrow(axis, (0.75, 0.48), (0.83, 0.50))
    axis.text(
        0.5,
        0.96,
        "Local–Global response-state intervention (only spatial aggregation changes)",
        ha="center",
        va="top",
        fontsize=12,
        fontweight="bold",
...
def _macro_arm_plot(
    ...
    *,
    ...
    if set(rows["arm"]) != set(ARM_ORDER) or rows["seed_base"].nunique() != 2:
        raise ValueError("macro arm plot requires two seeds and six arms")
    ...
    for index, arm in enumerate(ARM_ORDER):
        ...
        axis.scatter(
            np.full(len(values), index) + np.array([-0.07, 0.07]),
            values,
            s=48,
        ...
        axis.plot([index - 0.12, index + 0.12], [np.mean(values)] * 2, color="#111827", lw=2)
    ...
    axis.text(
        0.99,
        0.02,
        "dots: preregistered seeds; bars: two-seed mean",
...
def _two_task_macro_plot(
    ...
    *,
    ...
    for axis, table, task in zip(axes, (table2, table3), ("Static FTV", "Observed ΔFTV"), strict=True):
        ...
        for index, arm in enumerate(ARM_ORDER):
            ...
            axis.plot([index - 0.11, index + 0.11], [np.mean(values)] * 2, color="#111827", lw=1.8)
        if reference is not None:
            axis.axhline(reference, color="#94a3b8", linestyle="--", linewidth=1)
...
def paired_fold_plot(fold_effects: pd.DataFrame) -> plt.Figure:
    chosen = fold_effects.loc[
        fold_effects["comparison"].isin(("LOCAL0-GAP0", "LG0-LOCAL0"))
    ].copy()
    ...
    for axis, task in zip(axes, ("static", "delta"), strict=True):
        ...
        labels = ["LOCAL0−GAP0", "LG0−LOCAL0"]
        for index, comparison in enumerate(("LOCAL0-GAP0", "LG0-LOCAL0")):
            values = rows.loc[
            ...
            axis.scatter(
                ...
                values,
                color="#2a9d8f" if index == 0 else "#e9a03b",
                alpha=0.85,
                s=35,
            )
            axis.plot([index - 0.18, index + 0.18], [np.mean(values)] * 2, color="#111827", lw=2)
...
def safety_heatmap(table6: pd.DataFrame) -> plt.Figure:
    ...
    columns = [f"{seed}/F{fold}" for seed in (2026, 3026) for fold in range(5)]
