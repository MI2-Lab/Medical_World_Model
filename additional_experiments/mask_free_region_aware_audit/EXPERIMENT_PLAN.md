...
Schema-2 preregistration 已在 commit `c781b8f4c8ff14e0439e15447af9ca21bcd88be1`、lock SHA-256 `a574c905bd9780ac4260ca5e63a9a89e4db15a3217932409cda0fb00aaadd6ad` 冻结。本次 formal run 完成了 20/20 feature extraction、所有预注册 probe、paired bootstrap、Gate 和报告；在输出与 Gate 结果已被查看后，发现一个只影响公开 bootstrap summary 身份列的 implementation bug。`run_preregistered_bootstrap` 将 paired-bootstrap 返回行的 RNG `seed=260811` 在 dictionary expansion 时覆盖了 model seed，使 `metrics/table_bootstrap.csv` 的 222 行 `seed` 全部误写为 `260811`，而非各 111 行的 `2026` 与 `3026`。同表 `bootstrap_seed=260811` 正确；owner-only bootstrap draws 的 148,000 行 model-seed 身份（每个 seed 74,000 行）、重抽样、数值、CI 与每个 scientific cell 的关联均正确。Gate 代码不读取 bootstrap summary，因此 Gate 数值与 classification 未受影响。已查看的结果必须透明记录：Gate A=true，B=false，C=false，D=false，classification 为 `REGION_SIGNAL_EXISTS_BUT_NOT_BEYOND_FTV`；这些结果不得用于修改任何科学 contract、阈值或分析选择。

同期独立 static reporting audit 发现五个不改变数值、Gate 或科学结论的展示闭合问题：Figure 08 使用有偏的 head 截取且标签含糊；Figure 09 标签含糊；Figure 10 跨 context/population 混合平均；Figure 03 未显式展示 Gate-A population；报告 Q7 未分别明确回答 FTV 与 delta-FTV。允许的修正仅为：先验、对称选择 bootstrap 面板并显式标注 model seed/RNG seed；为 seed consistency 显式标注 context/population；将 timing sensitivity 按 context/population 分面而不混合平均；在 MRI-only pCR 图中明确标注 Gate-A 的 `ftv_complete_375` population；以分开句子报告 FTV 与 delta-FTV。

`PREREGISTRATION_IMPLEMENTATION_ERRATUM_2.json` 精确绑定完整 schema-2 run 当时的 94 个 active artifacts（790,736,881 bytes；sorted record-set SHA-256 `fc5a93acb2a0beca572c495fe6a60fecac5bce254dd74b79132911f326778853`）以及误写表、private draws、Gate、run summary、report manifest 与 feature completion 的精确 SHA-256。在 schema-3 refreeze 前，这 94 个 artifacts（包括 20 个 feature NPZ、20 个 metadata、completion marker、20 个 logs、所有 predictions/metrics/figures/reports）必须全部移出 active result tree；不得复用任何 superseded artifact，20/20 feature cells 也必须在 schema-3 lock 下重建。只允许 `.gitkeep` 与原始 `manifests/start_provenance.json` 留在 active result roots。

Schema 3 只允许修正 bootstrap summary 身份/provenance 与上述 static reporting/validation 闭合。Config、Goal5 selected cells、feature geometry/variants/projection、checkpoint/fold/order、population/timing、probe estimator/hyperparameter、metric、bootstrap draws/seed/CI、Oracle denominator/numerator、Gate threshold/logic、classification precedence、privacy 与 scientific contract 全部不变。Schema-3 lock 必须在任何 corrected extraction 或 label-dependent rerun 之前创建，并精确认证 schema-2 commit/lock、两个 append-only errata、本 plan 的 append-only disclosure、更新后的全部 scripts/tests 与 zero-result inventory。
