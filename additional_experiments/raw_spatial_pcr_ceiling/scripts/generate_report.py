...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate the Chinese Goal C report from aggregate metrics only.")
...
- 运行状态：`{decision.get('status', 'NOT_RUN')}`
- 预注册决策类：`{decision.get('decision_class', 'NOT_RUN')}`
