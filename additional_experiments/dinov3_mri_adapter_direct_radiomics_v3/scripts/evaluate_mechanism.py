...
from dinov3_rg.contracts import V2_TARGET_DIR, atomic_json  # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument("--target-root", type=Path, default=V2_TARGET_DIR)
    ...
    metrics, diagnostics = mechanism_metrics(args.target_root)
    metrics_path = ROOT / "metrics/mechanism_metrics.csv"
    metrics_path.parent.mkdir(parents=True, exist_ok=True)
    metrics.to_csv(metrics_path, index=False)
    diagnostics.to_csv(ROOT / "metrics/mechanism_probe_diagnostics.csv", index=False)
    ...
    if gate["status"] != "PASS":
        ...
        atomic_json(
            ROOT / "acceptance_check.json",
            {
                ...
                "representation_cells_completed": 50,
