...
from dinov3_rg.evaluation import (  # noqa: E402
    ...
    pooled_metrics, validate_oof_coverage,
)
...
def main() -> None:
    ...
    for seed in SEEDS:
        for fold in FOLDS:
            ...
            for arm in ARMS:
                ...
                if arm == "RAD" and not args.skip_radiomics_head_sensitivity:
                    predictions, diagnostics = evaluate_fold_cell(
                        fold_frame, state_path, seed=seed, fold=fold, arm=arm,
    ...
    mechanism_path = ROOT / "metrics/mechanism_metrics.csv"
    if not mechanism_path.is_file():
        raise RuntimeError("outcome-blind mechanism metrics are missing")
    mechanism = pd.read_csv(mechanism_path)
    privacy = public_artifact_privacy_scan(ROOT)
    safety_pass = (
        privacy["status"] == "PASS"
        and lock["cell_count"] == 50
        and lock["mechanism_lock"]["status"] == "LOCKED_FOR_PCR_EVALUATION"
    )
    ...
    acceptance = {
        ...
        "mechanism_lock_verified": True,
        "representation_cells": 50,
