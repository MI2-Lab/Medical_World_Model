...
V2_CHECKPOINT_ROOT = V2_ROOT / "checkpoints/formal"
V2_STATE_ROOT = V2_ROOT / "features/private/states"
...
VISITS = ("T0", "T1", "T2", "T3")
PILOT_SEED = 2026
PILOT_ARMS = ("R025", "R050", "R100")
PILOT_WEIGHTS = {"R025": 0.25, "R050": 0.50, "R100": 1.00}
...
def load_protocol(path: str | Path = PROTOCOL_PATH) -> dict[str, Any]:
    ...
    if payload.get("experiment") != "dinov3_mri_adapter_direct_radiomics_v3":
        raise ValueError("V3 experiment identity drifted")
    if tuple(payload["formal"]["seeds"]) != SEEDS:
        raise ValueError("formal seeds drifted")
    if tuple(payload["formal"]["folds"]) != FOLDS:
        raise ValueError("folds drifted")
    if tuple(payload["formal"]["arms"]) != ARMS:
        raise ValueError("arms drifted")
    if tuple(payload["pilot"]["candidate_arms"]) != PILOT_ARMS:
        raise ValueError("pilot arms drifted")
    ...
    if float(payload["loss"]["ftv"]) != 0.0:
        raise ValueError("V3 FTV loss must remain zero")
    if payload["loss"]["pilot_radiomics_weights"] != PILOT_WEIGHTS:
...
def validate_pilot_fold_arm(fold: int, arm: str) -> tuple[int, str]:
    if isinstance(fold, bool) or int(fold) not in FOLDS:
        raise ValueError("fold must be 0..4")
    normalized = str(arm).upper()
    if normalized not in PILOT_ARMS:
        raise ValueError(f"pilot arm must be one of {PILOT_ARMS}")
    return int(fold), normalized
...
__all__ = [
    ...
    "PILOT_ARMS", "PILOT_SEED", "PILOT_WEIGHTS", "PRIMARY_PATIENTS", "PROJECTION_SEEDS", "REPO_ROOT", "SEEDS", "TECHNICAL_ELIGIBILITY",
    ...
    "private_patient_token", "validate_pilot_fold_arm", "validate_seed_fold_arm", "verify_locked_file",
    "V2_CHECKPOINT_ROOT", "V2_ROOT", "V2_STATE_ROOT", "V2_SUMMARY_DIR", "V2_TARGET_DIR"
]
