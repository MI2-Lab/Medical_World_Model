#!/usr/bin/env python3
"""Run exact 100-cell feature export followed by sealed Ridge probes."""
...
def _resolve_default_data_contract(
    ...
    private_repository_root = (
        Path(configured).expanduser() if configured else REPO_ROOT
    ).resolve()
...
def _cells(
    ...
    matrix_identity = (
        tuple(str(arm) for arm in config["arms"]),
        tuple(int(seed) for seed in config["training"]["seed_bases"]),
        tuple(int(fold) for fold in config["training"]["folds"]),
    )
    if matrix_identity != (EXPECTED_ARMS, EXPECTED_SEEDS, EXPECTED_FOLDS):
        raise ValueError("postprocessing config is not the exact confirmation matrix")
    cells: list[Cell] = []
    ...
    if len(cells) != 100:
        raise AssertionError("postprocessing plan must contain exactly 100 cells")
    return tuple(cells)
...
def _validate_matrix(
    ...
    if (
        ...
        or int(completion.get("run_count", -1)) != 100
    ):
        raise ValueError("formal matrix completion does not authorize 100 cells")
    ...
    if not isinstance(runs, list) or len(runs) != 100:
        raise ValueError("matrix completion run inventory is not exactly 100 rows")
    observed = {
...
def main() -> None:
    ...
    if Path(str(getattr(pilot_security, "__file__", ""))).resolve() != (
        ...
        raise ImportError(
            "confirmation security module was shadowed before postprocessing"
        )
    ...
    from lg_response_pilot.analysis import load_confirmation_config
    ...
    checkpoints = resolve_contained_path(
