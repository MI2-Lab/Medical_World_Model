#!/usr/bin/env python3
"""Export one selected confirmation checkpoint's response state."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--arm",
        choices=("GAP0", "GAP3", "LOCAL0", "LOCAL3"),
        required=True,
    )
    parser.add_argument(
        "--seed-base",
        type=int,
        choices=(2026, 3026, 4026, 5026, 6026),
        required=True,
    )
...
def main() -> None:
    ...
    if Path(str(getattr(pilot_security, "__file__", ""))).resolve() != (
        ...
        raise ImportError(
            "confirmation security module was shadowed before feature export"
        )
