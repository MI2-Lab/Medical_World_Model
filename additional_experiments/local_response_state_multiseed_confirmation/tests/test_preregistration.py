...
class PreregistrationTest(unittest.TestCase):
    def test_config_and_upstream_hashes_are_closed(self) -> None:
        ...
        self.assertEqual(tuple(config["arms"]), ("GAP0", "GAP3", "LOCAL0", "LOCAL3"))
        self.assertEqual(
            tuple(config["training"]["seed_bases"]),
            (2026, 3026, 4026, 5026, 6026),
        )
        self.assertEqual(
            config["statistics"], FREEZE.EXPECTED_STATISTICS
        )
        self.assertEqual(
            config["gates"]["LOCAL_CONFIRMATION"][
        ...
        self.assertIn("不得重新调参", plan)
        self.assertIn("至少 23/25 paired folds PASS", plan)

    def test_code_inventory_covers_plan_config_and_scripts(self) -> None:
        inventory = FREEZE.code_inventory()
        suffixes = set(inventory)
        prefix = "additional_experiments/local_response_state_multiseed_confirmation/"
        self.assertIn(prefix + "EXPERIMENT_PLAN.md", suffixes)
        self.assertIn(prefix + "configs/confirmation.json", suffixes)
