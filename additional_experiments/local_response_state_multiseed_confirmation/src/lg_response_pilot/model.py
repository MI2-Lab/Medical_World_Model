"""Trainable GAP and fixed-local response-state confirmation models.
...
class LocalGlobalResponseWorldModel(DGRSWorldModel):
    """The sealed DGRS model with one confirmation pooling architecture.

    The historical class name is retained for checkpoint/import compatibility;
    the registered formal architectures are exactly GAP and LOCAL.
    """
    ...
    def __init__(
        ...
        _projection_parts(self.response_projection, FINAL_FEATURE_CHANNELS)
        ...
        # literal.  LOCAL keeps one shared deterministic buffer for every
    ...
    def _validate_sequence_inputs(
        ...
        if tuple(int(value) for value in image.shape[-3:]) != self.input_shape_zyx:
            raise ValueError(
                "confirmation image geometry must be frozen C1B-H "
                f"{self.input_shape_zyx} ZYX"
            )
        if validated_mask is not None:
            raise AssertionError("C1B confirmation unexpectedly admitted an ROI mask")
    ...
    def pooled_response_input(self, spatial: torch.Tensor) -> torch.Tensor:
        ...
        if self.response_architecture == "GAP":
            return spatial.mean(dim=(-3, -2, -1))
        if self.response_architecture == "LOCAL":
            return weighted_average_pool(spatial, self.local_weights_for(spatial))
    ...
    def _encode_sequence(
        ...
        if validated_mask is not None:
            raise AssertionError("confirmation model may not receive an ROI mask")
...
def shared_initialization_sha256(model: LocalGlobalResponseWorldModel) -> str:
    """Hash every shared module that must initialize identically in all arms."""
...
def validate_model_contract(
    ...
    if model.requires_roi_mask:
        raise ValueError("confirmation models must never accept an ROI mask")
    ...
    if spec.architecture == "GAP":
        ...
        *derived_shape,
        ...
        raise ValueError("LOCAL fixed local-weight buffer shape drifted")
...
def paired_initialization_report(effective_seed: int) -> dict[str, Any]:
    """Build all four arms and prove the frozen paired initialization contract."""
    ...
    projection_hashes = {
        record["response_projection_sha256"] for record in per_arm.values()
    }
    target_projection_hashes = {
        record["target_response_projection_sha256"] for record in per_arm.values()
    }
    grounded_head_hashes = {
        per_arm[arm]["ftv_head_sha256"] for arm in ("GAP3", "LOCAL3")
    }
    checks = {
        "all_shared_modules_identical": len(shared_hashes) == 1,
        "all_transitions_identical": len(transition_hashes) == 1,
        "all_online_response_projections_identical": len(projection_hashes) == 1,
        "all_target_response_projections_identical": len(target_projection_hashes)
        == 1,
        "gap0_local0_initialization_paired": per_arm["GAP0"][
            "shared_initialization_sha256"
        ]
        == per_arm["LOCAL0"]["shared_initialization_sha256"],
        "gap3_local3_initialization_paired": per_arm["GAP3"][
            "shared_initialization_sha256"
        ]
        == per_arm["LOCAL3"]["shared_initialization_sha256"],
    ...
    return {
        ...
        "baseline_response_projection_sha256": per_arm["GAP0"][
            "response_projection_sha256"
        ],
        "architecture_pairs": {
            "GAP0_LOCAL0": per_arm["GAP0"]["shared_initialization_sha256"],
            "GAP3_LOCAL3": per_arm["GAP3"]["shared_initialization_sha256"],
        },
        "per_arm": per_arm,
        "checks": checks,
    }
...
def load_checkpoint_for_evaluation(
    ...
    """Strictly load one selected, test-blind confirmation checkpoint."""
