...
FEATURE_DIMS = {
    "T0": RESPONSE_DIM,
    "T0-T1": 3 * RESPONSE_DIM,
    "T0-T2": 6 * RESPONSE_DIM,
}
LOGISTIC_SOLVER = "liblinear"
LOGISTIC_MAX_ITER = 20_000
LOGISTIC_TOL = 1e-7
PCR_READOUT_SEED = 2026
...
def response_readout_matrix(
    response_state: np.ndarray, decision_point: str
) -> np.ndarray:
...
def run_pcr_readouts(
    *,
    ...
    """运行一个 seed_base×model×fold 的三个独立 pCR readout。"""
    ...
    penalties = tuple(str(value) for value in penalties)
    c_grid = tuple(float(value) for value in c_grid)
    if penalties != PENALTIES or c_grid != C_GRID:
        raise ValueError(f"本实验 pCR penalty/C grid 必须锁定为 {PENALTIES}/{C_GRID}")
    ...
    for decision_index, decision_point in enumerate(DECISION_POINTS):
        rows, selection = _run_decision_point(
            ...
            penalties=penalties,
            c_grid=c_grid,
            random_state=int(PCR_READOUT_SEED + fold * 100 + decision_index),
        )
    ...
    for frame in (predictions, selections):
        if (
            ...
            raise ValueError("pCR 输出 seed/fold contract 漂移")
    ...
    if (
        predictions[list(selection_flags)].to_numpy(dtype=bool).any()
        or selections[list(selection_flags)].to_numpy(dtype=bool).any()
    ):
        raise ValueError("pCR 输出声称 test 参与了选择/拟合")
    for frame in (predictions, selections):
        if (
            not frame["test_feature_matrix_constructed_after_selection_lock"]
            .eq(True)
            .all()
        ):
    ...
    summary = {
        ...
        "readout_rng_base_seed": PCR_READOUT_SEED,
        ...
        "logistic": {
            ...
            "random_state_schedule": (
                f"{PCR_READOUT_SEED} + fold*100 + decision_index; "
                "independent of encoder seed_base"
            ),
            "class_weight": "balanced",
...
def validate_pcr_asset(
    *,
    ...
    """严格验证一个已完成 pCR 资产；仅供安全 resume 跳过使用。"""
    ...
    if not all(path.is_file() for path in required):
        raise FileNotFoundError(f"pCR 资产不完整：{required}")
    ...
    if identity != (model_name, seed_base, fold, seed_base + fold):
        raise ValueError(f"pCR summary identity 错位：{identity}")
    if int(summary.get("schema_version", -1)) != 2:
        raise ValueError("pCR summary schema_version 非 2")
    if summary.get("readout_rng_base_seed") != PCR_READOUT_SEED:
        raise ValueError("pCR readout RNG 基准种子不是锁定的 2026")
    if summary.get("decision_points") != list(DECISION_POINTS):
        raise ValueError("pCR decision points 漂移")
    if summary.get("prediction_columns") != list(PREDICTION_COLUMNS) or summary.get(
        "selection_columns"
    ) != list(SELECTION_COLUMNS):
        raise ValueError("pCR summary column schema 漂移")
    if summary.get("prediction_file_sha256") != file_sha256(prediction_path):
        raise ValueError("pCR prediction SHA 不匹配")
    if summary.get("selection_file_sha256") != file_sha256(selection_path):
        raise ValueError("pCR selection SHA 不匹配")
    if (
        summary.get("source_feature_sha256") != asset.sha256
        or summary.get("source_checkpoint_sha256")
        != asset.metadata["checkpoint_sha256"]
    ):
        raise ValueError("pCR source feature/checkpoint provenance 漂移")
    if summary.get("pcr_implementation_sha256") != pcr_implementation_sha256():
        raise ValueError("pCR implementation SHA 与当前冻结代码不一致")
    ...
    if (
        ...
        raise ValueError("pCR CSV column schema 漂移")
    prediction_key = [
        "patient_id",
        "seed_base",
        "fold",
    ...
    selection_key = prediction_key[1:]
    if predictions.empty or predictions.duplicated(prediction_key).any():
        raise ValueError("pCR prediction key 不唯一")
    if len(selections) != 3 or selections.duplicated(selection_key).any():
        raise ValueError("pCR selection key/decision point 不完整")
    if set(selections["decision_point"].astype(str)) != set(DECISION_POINTS) or set(
        predictions["decision_point"].astype(str)
    ) != set(DECISION_POINTS):
        raise ValueError("pCR CSV decision point 集合不完整")
    for frame in (predictions, selections):
        if (
            not frame["seed_base"].eq(seed_base).all()
            or not frame["fold"].eq(fold).all()
            ...
            raise ValueError("pCR CSV identity 漂移")
    probabilities = predictions["probability"].to_numpy(dtype=np.float64)
    if not np.isfinite(probabilities).all() or np.any(
        (probabilities < 0.0) | (probabilities > 1.0)
    ):
        raise ValueError("pCR CSV probability 非法")
    expected_random_states = {
        point: PCR_READOUT_SEED + fold * 100 + index
        for index, point in enumerate(DECISION_POINTS)
    }
    observed_random_states = dict(
        zip(
            selections["decision_point"].astype(str),
            selections["random_state"].astype(int),
            strict=True,
        )
    )
    if observed_random_states != expected_random_states:
        raise ValueError("pCR per-decision random_state 与锁定基准 2026 不一致")
    if len(predictions) != int(summary.get("test_prediction_rows", -1)):
        raise ValueError("pCR prediction row count 与 summary 不一致")
    return summary


def synthetic_self_test() -> dict[str, Any]:
    if MODELS != ("G1", "G3") or PCR_READOUT_SEED != 2026:
        raise AssertionError("pCR scope/seed contract 漂移")
    seed_fields = {"seed_base", "fold", "effective_seed"}
    if not seed_fields.issubset(PREDICTION_COLUMNS) or not seed_fields.issubset(
        SELECTION_COLUMNS
    ):
        raise AssertionError("pCR CSV schema 缺 seed contract")
    ...
    try:
        guard.predict_positive_probability(
            selected.model, selected.scaler.transform(test)
        )
    except RuntimeError:
    ...
    return {
        ...
        "readout_seed_base_locked": PCR_READOUT_SEED,
        "models_locked": list(MODELS),
        "penalties_locked": list(PENALTIES),
        "C_grid_locked": list(C_GRID),
