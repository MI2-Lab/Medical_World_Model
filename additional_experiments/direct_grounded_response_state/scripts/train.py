#!/usr/bin/env python3
"""训练一个 G1–G4 fold；支持 smoke、lambda pilot 与 baseline-aware selection。"""
...
def main() -> None:
    ...
    parser.add_argument("--model-name", "--model", dest="model_name", choices=("G1", "G2", "G3", "G4"), required=True)
    parser.add_argument("--device", default="cuda")
    parser.add_argument("--epochs", type=int)
    parser.add_argument("--workers", type=int)
    parser.add_argument("--smoke-patients", type=int)
    parser.add_argument("--lambda-ftv", type=float)
    ...
    checkpoint = train_fold(
        ...
        lambda_ftv_override=args.lambda_ftv,
