"""G1–G4 的 test-blind 五折训练、诊断与 checkpoint 选择。"""
...
from .model import DGRSOutput, DGRSWorldModel, GROUNDED_MODELS, VALID_MODELS
...
def git_metadata() -> dict[str, str]:
    ...
    return {
        ...
        "experiment_status": run(
            "git", "status", "--short", "--", "additional_experiments/direct_grounded_response_state"
        ),
    }
...
def validate_run_config(config: Mapping[str, Any]) -> None:
    ...
    loss = config["loss"]
    train = config["train"]
    if str(model.get("model_name", "")).upper() not in VALID_MODELS:
        raise ValueError(f"model.model_name 必须是 {VALID_MODELS}")
    if int(model.get("image_channels", 7)) != 7:
        raise ValueError("G1–G4 image_channels 必须为 7")
...
def _checkpoint_payload(
    ...
    return {
        ...
        "determinism": {
            ...
            "fixed_patient_order_seed": int(runtime["seed"]),
...
def train_fold(
    ...
    if lambda_ftv_override is not None:
        config["loss"]["lambda_ftv"] = float(lambda_ftv_override)
    ...
    lambda_ftv = float(config["loss"]["lambda_ftv"]) if model_name in GROUNDED_MODELS else 0.0
    config["loss"]["lambda_ftv"] = lambda_ftv
    if model_name not in GROUNDED_MODELS and lambda_ftv_override not in (None, 0, 0.0):
        raise ValueError(f"{model_name} 不允许非零 lambda_ftv")
    ...
    if baseline_path is not None:
        ...
        if str(baseline_payload.get("model_name")) not in ({"G1"} if model_name == "G3" else {"G2"}):
    ...
    if model_name in GROUNDED_MODELS:
        ...
        raise ValueError("G1/G2 baseline 训练不接受 baseline selection 参数")
    ...
    if smoke_patients is not None:
        train_ids = _smoke_subset(list(splits["train"]), bundle.raw_ftv, smoke_patients, seed)
        val_ids = _smoke_subset(list(splits["val"]), bundle.raw_ftv, max(2, smoke_patients // 2), seed + 1)
    ...
    train_loader = make_loader(
        records_for_ids(bundle, train_ids), transformed, bundle.raw_ftv, batch_size, workers, True, seed
    )
    val_loader = make_loader(
        records_for_ids(bundle, val_ids), transformed, bundle.raw_ftv, batch_size, workers, False, seed
    )
    ...
    baseline_contract = {
        "paired_model": "G1" if model_name == "G3" else "G2" if model_name == "G4" else None,
