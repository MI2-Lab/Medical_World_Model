"""DGRS 冻结 observed response-state 特征提取与资产验证。

本模块刻意不读取 FTV/LD/sphericity target。G0 仅物化已完成 OSRA audit
中 M0 的 ``online_preprojector``；G1--G4 则从冻结 checkpoint 的
``encode_response`` 提取 ``r``。正式资产统一为 ``[808,4,192]``，患者顺序
严格是每个 outer fold 的 train、validation、test 顺序。
"""
...
OSRA_ROOT = REPO_ROOT / "additional_experiments" / "observed_state_radiomics_audit"
...
class _FrozenDCE7Dataset(Dataset[dict[str, Any]]):
    ...
    def __init__(self, patient_ids: Sequence[str], cache: Mapping[str, Path]) -> None:
        ...
        missing = [patient_id for patient_id in self.patient_ids if patient_id not in cache]
        if missing:
...
def _safe_payload(path: Path) -> Mapping[str, Any]:
    try:
        ...
        raise RuntimeError(f"checkpoint 无法通过 weights_only 安全加载：{path}") from error
...
def _validate_checkpoint_contract(
    ...
    if int(model_config.get("image_channels", 7)) != 7:
        raise ValueError("G1--G4 checkpoint 必须锁定 7-channel backbone")
    ...
    for split, expected in expected_by_split.items():
        ...
        if [str(value) for value in actual] != expected:
            raise ValueError(f"checkpoint {split} patient 顺序与 canonical manifest 不一致")
...
def _encode_response(
    ...
    if model_name in {"G1", "G3"}:
        response = encoder(image)
    elif model_name in {"G2", "G4"}:
        try:
            response = encoder(image, roi_mask=roi_mask)
        except TypeError:
            response = encoder(image, roi_mask)
    else:
...
def _normalise_checkpoint_path(
    ...
    for run_name in (
        model_name,
        model_name.lower(),
        f"{model_name.lower()}_final",
        f"{model_name.lower()}_lambda_selected",
    ):
        candidate = DGRS_ROOT / "checkpoints" / run_name / f"fold_{fold}" / "best.pt"
        if candidate.is_file():
            candidates.append(candidate.resolve())
    if not candidates:
        candidates = sorted(
            path.resolve()
            for path in (DGRS_ROOT / "checkpoints").glob(
        ...
        for path in candidates:
            try:
                if _payload_model_name(_safe_payload(path)) == model_name:
    ...
    if len(candidates) != 1:
        raise FileNotFoundError(
            f"无法唯一解析 {model_name}/fold_{fold} best.pt；"
            f"候选={list(map(str, candidates))}，请显式传 --checkpoint"
        )
    return candidates[0]
...
def _validate_feature_rows(
    ...
    if not np.array_equal(actual_labels.astype(np.int64, copy=False), expected_labels_array):
...
def _write_asset(
    *,
    ...
    output_dir = Path(output_root) / model_name / f"fold_{fold}"
    ...
    metadata.update(
        {
            ...
            **canonical_evidence,
            ...
            "inference_inputs": (
                ["DCE7", "binary_roi_mask_channel"]
                if model_name == "G0"
                else ["DCE7"]
                + (["normalized_roi_pooling_mask"] if model_name in {"G2", "G4"} else [])
            ),
    ...
    if rows.duplicated(["patient_id", "fold", "model"]).any():
...
def materialize_g0(
    *,
    fold: int,
    output_root: Path = DGRS_ROOT / "features",
    osra_root: Path = OSRA_ROOT,
    fold_manifest: Path = DEFAULT_FOLD_MANIFEST,
    max_patients_per_split: int | None = None,
    overwrite: bool = False,
) -> dict[str, Any]:
    """从 OSRA M0 ``online_preprojector`` 物化 G0 response state。"""

    _, canonical_ids, canonical_splits, canonical_labels = _canonical_fold(
        fold_manifest, fold
    )
    source_dir = Path(osra_root) / "features" / "m0" / f"fold_{fold}"
    source_path = source_dir / "observed_features.npz"
    source_metadata_path = source_dir / "extraction_metadata.json"
    if not source_path.is_file() or not source_metadata_path.is_file():
        raise FileNotFoundError(f"缺少 OSRA G0 source asset：{source_dir}")
    source_metadata = json.loads(source_metadata_path.read_text(encoding="utf-8"))
    source_hash = file_sha256(source_path)
    if source_hash != source_metadata.get("feature_file_sha256"):
        raise ValueError("OSRA source feature SHA 与 metadata 不一致")
    with np.load(source_path, allow_pickle=False) as archive:
        if "online_preprojector" not in archive:
            raise KeyError("OSRA source 缺 online_preprojector")
        source_ids = archive["patient_ids"].astype(str)
        source_splits = archive["splits"].astype(str)
        response = archive["online_preprojector"].astype(np.float32, copy=True)
        source_timepoints = archive["timepoints"].astype(str)
    if tuple(source_ids) != canonical_ids or tuple(source_splits) != canonical_splits:
        raise ValueError("OSRA G0 patient IDs/split 顺序与 canonical manifest 不一致")
    if tuple(source_timepoints) != TIMEPOINTS or response.shape != (808, 4, 192):
        raise ValueError("OSRA G0 response contract 漂移")
    indices = np.arange(808)
    if max_patients_per_split is not None:
        if max_patients_per_split <= 0:
            raise ValueError("max_patients_per_split 必须为正整数")
        indices = np.concatenate(
            [
                np.flatnonzero(source_splits == split)[:max_patients_per_split]
                for split in ("train", "val", "test")
            ]
        )
    arrays = {
        "patient_ids": source_ids[indices],
        "splits": source_splits[indices],
        "response_state": response[indices],
        "timepoints": np.asarray(TIMEPOINTS, dtype=str),
        "model": np.asarray("G0"),
        "fold": np.asarray(fold, dtype=np.int64),
        "label_pcr": canonical_labels[indices].astype(np.int64),
    }
    metadata = {
        "source_kind": "reused_osra_online_preprojector",
        "source_feature_file": str(source_path.resolve()),
        "source_feature_sha256": source_hash,
        "source_feature_metadata": str(source_metadata_path.resolve()),
        "source_feature_metadata_sha256": file_sha256(source_metadata_path),
        ...
        "source_extractor_sha256": source_metadata.get("extractor_sha256"),
        "source_config": source_metadata.get("source_config"),
        "source_config_sha256": source_metadata.get("source_config_sha256"),
        "source_raw_radiomics_sha256": source_metadata.get("raw_radiomics_sha256"),
        "architecture_contract": "legacy M0 DCE7+mask channel -> GAP -> online pre-projector",
...
def extract_trained_model(
    *,
    ...
    """从 G1--G4 best checkpoint 提取全部 primary patient 的 frozen ``r``。"""
    ...
    metadata = {
        ...
        "checkpoint_resolved_config_sha256": payload.get(
            "resolved_config_sha256"
        ),
        "shared_initialization_sha256": payload.get(
        ...
        "device": str(device),
        "online_encoder": True,
        "ftv_head_loaded_but_not_called": model_name in {"G3", "G4"},
    }
    del model
    if device.type == "cuda":
...
def extract_model(
    *,
    ...
    if model_name == "G0":
        if checkpoint is not None:
            raise ValueError("G0 必须复用 OSRA feature，不接受新 checkpoint")
        return materialize_g0(
            fold=fold,
            output_root=output_root,
            osra_root=osra_root,
...
def synthetic_self_test() -> dict[str, Any]:
    ...
    arrays = {
        ...
        "splits": splits,
        "response_state": generator.normal(
            size=(sum(counts), 4, RESPONSE_DIM)
        ).astype(np.float32),
