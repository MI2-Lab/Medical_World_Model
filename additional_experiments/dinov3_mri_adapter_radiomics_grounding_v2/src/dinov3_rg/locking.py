...
def freeze_evaluation_lock(
    ...
    *,
    ...
    prerequisite_paths = [
        ...
        EXPERIMENT_ROOT / "metrics/roi_feasibility.json",
        EXPERIMENT_ROOT / "target_feasibility.json",
...
def freeze_mechanism_lock(
    ...
    *,
    ...
    if MECHANISM_LOCK_PATH.exists() and not overwrite:
        raise FileExistsError(f"mechanism lock already exists: {MECHANISM_LOCK_PATH}")
    representation = verify_representation_lock()
    gate_path = Path(gate_path)
    metrics_path = Path(metrics_path)
    if not gate_path.is_file() or not metrics_path.is_file():
        raise FileNotFoundError("mechanism gate artifacts are incomplete")
    gate = json.loads(gate_path.read_text(encoding="utf-8"))
    if gate.get("status") != "PASS" or not all(gate.get("gates", {}).values()):
        raise PermissionError("mechanism gate did not pass; pCR remains locked")
    payload = {
        "schema_version": 1,
    ...
    payload["lock_content_sha256"] = canonical_sha256(payload)
    atomic_json(MECHANISM_LOCK_PATH, payload)
    return payload
...
def verify_mechanism_lock(path: str | Path = MECHANISM_LOCK_PATH) -> dict[str, Any]:
    ...
    source = Path(path)
    if not source.is_file():
        raise PermissionError("pCR evaluation is locked: MECHANISM_LOCK.json is absent")
    payload = json.loads(source.read_text(encoding="utf-8"))
    claimed = payload.pop("lock_content_sha256", None)
    valid = (
        payload.get("status") == "LOCKED_FOR_PCR_EVALUATION"
        and claimed == canonical_sha256(payload)
        and payload.get("representation_lock_sha256") == file_sha256(LOCK_PATH)
        and payload.get("representation_lock_content_sha256") == representation["lock_content_sha256"]
        and all(payload.get("gates", {}).values())
    )
    if not valid:
        raise PermissionError("mechanism lock content/hash is invalid")
    payload["lock_content_sha256"] = claimed
    return payload
...
__all__ = [
    "LOCK_PATH", "MECHANISM_LOCK_PATH", "freeze_evaluation_lock", "freeze_mechanism_lock",
    "implementation_manifest", "verify_evaluation_lock", "verify_mechanism_lock",
