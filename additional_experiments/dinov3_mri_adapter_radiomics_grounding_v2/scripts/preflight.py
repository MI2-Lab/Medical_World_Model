...
from dinov3_rg.radiomics import morphology_variants  # noqa: E402
...
def main() -> None:
    ...
    v1_root = ROOT.parent / "dinov3_mri_adapter_radiomics_grounding"
    v1_immutable = {
        "decision": file_sha256(v1_root / "decision.json") == protocol["parent"]["v1_decision_sha256"],
        "report": file_sha256(v1_root / "reports/final_report.md") == protocol["parent"]["v1_report_sha256"],
        "protocol": file_sha256(v1_root / "configs/protocol.json") == protocol["parent"]["v1_protocol_sha256"],
    }
    ...
    morphology_mask = np.zeros((32, 72, 72), dtype=bool)
    morphology_mask[10:13, 30:35, 30:35] = True
    morphology_valid = np.ones_like(morphology_mask)
    morphology_valid[10:13, 29, 30:35] = False
    _, _, bounded_dilation = morphology_variants(morphology_mask, morphology_valid)
    try:
    ...
    representation_paths = [
        ...
        ROOT / "src/dinov3_rg/mechanism.py",
        ...
        ROOT / "scripts/run_smoke_cell.py",
        ROOT / "scripts/evaluate_mechanism.py",
    ]
    source_scan = scan_representation_sources(representation_paths)
    gates = {
        "bowen_environment": True,
        "locked_inputs": len(locked) == 6,
        "v1_artifacts_immutable": all(v1_immutable.values()),
        ...
        "morphology_bounded_by_valid_source": not bool((bounded_dilation & ~morphology_valid).any()),
        "grounding_visits_t0_t2": tuple(protocol["radiomics"]["grounding_visits"]) == ("T0", "T1", "T2"),
