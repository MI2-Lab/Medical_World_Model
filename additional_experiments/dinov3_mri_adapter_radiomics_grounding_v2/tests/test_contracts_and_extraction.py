...
def test_protocol_matrix_and_summary_contract():
    ...
    assert tuple(protocol["radiomics"]["grounding_visits"]) == ("T0", "T1", "T2")
    assert protocol["radiomics"]["minimum_voxels"] == 64
    assert protocol["radiomics"]["pyradiomics_minimum_roi_size_setting"] == 63
    assert protocol["radiomics"]["coverage_minimum"]["T2"] == 0.85
